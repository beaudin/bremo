...
public class KivaPostProcessingThread implements Callable{
	...
	public void postProcess(){
		...
		double lambdaTot=allCellSpec.get_lambda();
		...
		for(int cellIdx=0;cellIdx<ggVec.length;cellIdx++){
			if(ggVec[cellIdx].get_lambda()<lambdas[0]){
				...
				vol[0]=vol[0]+kpr.get_V(cellIdx);
				mass[0]=mass[0]+kpr.get_m(cellIdx);	
			}
			for(int i=0;i<lambdas.length-1;i++){
				if(ggVec[cellIdx].get_lambda()>=lambdas[i]&&
					...
					vol[i]=vol[i]+kpr.get_V(cellIdx);
					mass[i]=mass[i]+kpr.get_m(cellIdx);
				}
			}
			if(ggVec[cellIdx].get_lambda()>=lambdas[lambdas.length-1]){
				vecsIdx[nbrOfBins-1].add(cellIdx);
				vol[nbrOfBins-1]=vol[nbrOfBins-1]+kpr.get_V(cellIdx);
				mass[nbrOfBins-1]=mass[nbrOfBins-1]+kpr.get_m(cellIdx);
			}
		}
	...
	private void createBremoMultiZoneInput(int nbrOfZones){
		...
		for(int cellIdx=0;cellIdx<ggVec.length;cellIdx++){						
				if(ggVec[cellIdx].get_lambda()>lamMax)
					lamMax=ggVec[cellIdx].get_lambda();
				if(ggVec[cellIdx].get_lambda()<lamMin)
					lamMin=ggVec[cellIdx].get_lambda();	
		}		
		...
		for(int cellIdx=0;cellIdx<ggVec.length;cellIdx++){
			for(int i=0;i<lam4Zones.length-1;i++){
				if(ggVec[cellIdx].get_lambda()>=lam4Zones[i]&&
						ggVec[cellIdx].get_lambda()<lam4Zones[i+1]){
					vecsIdx[i].add(cellIdx);					
				}
			}
			if(ggVec[cellIdx].get_lambda()>=lam4Zones[lam4Zones.length-1]){
		...
		//creating the zones
		double p=kpr.get_p();	
		double lamLHVmTotpVTmi[][]=new double [nbrOfZones][ggVec[0].get_speziesMassenBruecheDetail().size()+6];
		//Header  for output
		String header[]=new String [ggVec[0].get_speziesMassenBruecheDetail().size()+6];
		header[0]="Lambda [-]";
		header[1]="LHV [MJ/kg]";
		header[2]="mTot [kg]";
		header[3]="p [Pa]";
		header[4]="V [m^3]";
		header[5]="T [K]";
		//needed to only output zones that really exist!
		int writeIdx=0;
...
//		}	
	...
	public Vector<Integer>[] get_biIdx(){
		return this.vecsIdx;
	}
