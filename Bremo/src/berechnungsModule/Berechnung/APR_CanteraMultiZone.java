...
/*public class APR_CanteraMultiZone extends APR_Cantera 		
	private Motor motor;
	...
	protected APR_CanteraMultiZone(CasePara cp) {
		...
		indexOfMainInj=CP.get_nbrOfMainInj()-1;
	 ...
	 * @param zonen_IN
	 */
/*	private void hs_0(double time, Zone[] zonen_IN){
	 ...
	 * @param zonen_IN
	 */
/*	private void hs_1(double time, Zone[] zonen_IN){
		for(int i=0;i<masterEinspritzung.get_AllInjections().length;i++){	
			Einspritzung es=masterEinspritzung.get_AllInjections()[i];					
			if(es.get_Krst()==CP.SPEZIES_FABRIK.get_nC7H16()){		
...
/*	private double packetMix=0.0;
	private void hs_2(double time, Zone[] zonen_IN){
		packetMix=0.0;
		for(int i=0;i<masterEinspritzung.get_AllInjections().length;i++){	
			Einspritzung es=masterEinspritzung.get_AllInjections()[i];					
			if(es.get_Krst()==CP.SPEZIES_FABRIK.get_nC7H16()){	
	 ...
	 * @param zonen_IN
	 */
/*	private void hs_3(double time, Zone[] zonen_IN){
		for(int i=0;i<masterEinspritzung.get_AllInjections().length;i++){	
			Einspritzung es=masterEinspritzung.get_AllInjections()[i];					
			if(es.get_Krst()==CP.SPEZIES_FABRIK.get_nC7H16()){	
	 ...
	 * @param zonen_IN
	 */	
/*	private void hs_4(double time, Zone[] zonen_IN){
		packetMix=0d;	
		for(int i=0;i<masterEinspritzung.get_AllInjections().length;i++){	
			Einspritzung es=masterEinspritzung.get_AllInjections()[i];					
			if(es.get_Krst()==CP.SPEZIES_FABRIK.get_nC7H16()){
	 ...
	 * @param zonen_IN
	 */
/*	private void hs_5(double time, Zone[] zonen_IN){
		for(int i=0;i<masterEinspritzung.get_AllInjections().length;i++){	
			Einspritzung es=masterEinspritzung.get_AllInjections()[i];					
			if(es.get_Krst()==CP.SPEZIES_FABRIK.get_nC7H16()){	
				int axialPakete=((Spray) es).get_AnzahlAxialPakete();
	...
	private double calc_dm(Zone z0,Zone z1, double time){
		boolean hanz=false;
		if(hanz)
			return this.calc_dma_Hanz(z0, z1, time);
		else 
	...
	private double calc_dma_Hanz(Zone z0,Zone z1, double time){	
		double exp_m=0.8;
		...
		double querschnitt = Math.pow(((Motor_HubKolbenMotor) motor).get_Bohrung(),2)/4*Math.PI ;
		double vh = ((Motor_HubKolbenMotor) motor).get_EV_Hub(); // [m]
		double vh_max = ((Motor_HubKolbenMotor) motor).get_EV_Hub_max(); // [m]

		double einstroem =  1.9*(mluft / dichte / querschnitt)*Math.pow((vh_max / vh),1.35);
		einstroem=0;
		double mittlereKolbengeschwindigkeit=CP.get_DrehzahlInUproSec()*((Motor_HubKolbenMotor) motor).get_Hub()*2; //[m/s]
		double momentanegeschw = ((Motor_HubKolbenMotor) motor).get_Kolbengeschwindigkeit(time);
		double geschwindigkeit = einstroem+Math.sqrt(Math.pow(momentanegeschw,2) + Math.pow(mittlereKolbengeschwindigkeit,2))/2;
		double geschwindigkeitsterm = C3 * Math.pow(geschwindigkeit, exp_m);		
		...
		double LL= Math.pow(Vzyl, exp );							
		//charachteristic length scale
		double C4=CP.get_C_MixRad();	
		double lc=C4*Math.pow(V0, (1d/3d));	
		
		//eigentlich richtig! Vielleicht statt der eins eine 0.005 oder so
		double factor=Math.exp(-1.0*(lc/LL-1.0)*(lc/LL-1.0));
		//surface 
		double surf=Math.pow(V0, 2d/3d);		
		double C_Mix=CP.get_C_Mix();
		double dma=C_Mix*Math.sqrt(rho_0*rho_1)*geschwindigkeitsterm*surf;	
		//this makes nearly no differnce
		//dma=C_Mix*rho_0*ue*surf;
	...
	private Zone[] turbMix1(double time, Zone[] zonen_IN){
		...
		if(time<=es.get_EOI()){
			...
			int ax=0;
			int rad=0;
			//ratio between spray axis and spray rim --> dm0/dmR=exp(a*(R-r)^2
			double a=Math.log(0.5)/radialPakete/radialPakete;
			do{						
				do{
					...
					double dma=this.calc_dm(z0, z1, time);	
					//doesn't work!
					double f=Math.exp(a*Math.pow(radialPakete-rad, 2));
					dma=f*dma;
					
					//check if gasoline zone still conains enough mass
					if(z1.get_m()-CP.SYS.MINIMALE_ZONENMASSE >=(dmaTot+dma)*CP.SYS.WRITE_INTERVAL_SEC
							&& z1.get_m()>=CP.SYS.MINIMALE_ZONENMASSE){
						dmaTot=dmaTot+dma;
						z0.set_dm_ein(dma, z1.get_T(), z1.get_ggZone());
						try {
...
//					double f=0.9;
//					double dm0=dm;
//					if(zonen_IN[z0].get_m()-CP.SYS.MINIMALE_ZONENMASSE<=dm*CP.SYS.WRITE_INTERVAL_SEC
//							&&zonen_IN[z0].get_m()>CP.SYS.MINIMALE_ZONENMASSE)				
//						dm0=f*(zonen_IN[z0].get_m()-CP.SYS.MINIMALE_ZONENMASSE)/CP.SYS.WRITE_INTERVAL_SEC;
//					if(zonen_IN[z0].get_m()<=CP.SYS.MINIMALE_ZONENMASSE)
//						dm0=0;
//
//					double dm1=dm;
//					if(zonen_IN[z0+1].get_m()-CP.SYS.MINIMALE_ZONENMASSE<=dm*CP.SYS.WRITE_INTERVAL_SEC
//							&&zonen_IN[z0+1].get_m()>CP.SYS.MINIMALE_ZONENMASSE)						
//						dm1=f*(zonen_IN[z0+1].get_m()-CP.SYS.MINIMALE_ZONENMASSE)/CP.SYS.WRITE_INTERVAL_SEC;								
//					if(zonen_IN[z0+1].get_m()<=CP.SYS.MINIMALE_ZONENMASSE)
//						dm1=0;
//
//					dm=Math.min(dm1, dm0);								
...
//						zonen_IN[z0+1].set_dm_aus(dm);
//					} catch (NegativeMassException e) {
//						e.log_Warning("Zone "+ (z0+1) +" is empty! Something went wrong");
//						//Passiert eh nicht!!
//					}
//					
//					//diffusion into zone r+1
//					zonen_IN[z0+1].set_dm_ein(dm, 
//							zonen_IN[z0].get_T(), 
//							zonen_IN[z0].get_ggZone());
//					try {
...
//					}				
	...
	public void bufferErgebnisse(double time, Zone[] zn) {
		...
		i+=1;
		super.buffer_EinzelErgebnis("lamGesMix [-]",gg.get_lambda(),i);
		
		double doh=0; //Degree of heterogeneity
		for(int iterZn=0; iterZn<zn.length;iterZn++){
			double a=zn[iterZn].get_ggZone().get_lambda()-lamGesSum;
			doh=doh+a*a*zn[iterZn].get_m();
		}
		doh=Math.sqrt(doh/sum_m)/lamGesSum;
		...
		int nbrLam=10;
		double lMax=Math.round(this.lambdaMax)+0.5;		
		//System.out.println(lMax);
		double a=0d;
		double lamInt=(lMax-a*lMax)/nbrLam;
		//System.out.println(lamInt);
		double lambdas []= new double [nbrLam+1];
		double masses[]=new double [nbrLam];
		for(int j=0;j<=nbrLam;j++){
			lambdas [j]=lMax-j*lamInt;
		}
		...
		i+=1;
		super.buffer_EinzelErgebnis("TKE [m^2/s^2]", this.tfr.get_TKE(time),i);

		i+=1;
		super.buffer_EinzelErgebnis("EPS [m^2/s^3]", this.tfr.get_EPS(time),i);

		i+=1;
		super.buffer_EinzelErgebnis("TI [m/s]", this.tfr.get_TI(time),i);
