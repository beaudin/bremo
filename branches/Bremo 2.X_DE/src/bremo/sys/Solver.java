...
public class Solver implements DglSysFunction {
	...
	public void set_BerechnungsModell(BerechnungsModell bmi){
		this.bmi=bmi;
	}
	
	public Zone[] solveRK(Zone [] znIN){		
		int numOfValues=3+CP.SPEZIES_FABRIK.get_nbrOfSpecies(); //p,T,V + Speziesmassen
...
//				p_V_T_mi_RK=this.rk.fehlberg(this);
	...
	private double[] get_dp_dV_dT_dmi_dParas(Zone [] zonen, double time){	
		...
		int numOfValues=3+CP.SPEZIES_FABRIK.get_nbrOfSpecies();
		int nbrOfParameters=0;
		for(int i=0;i<vecMPI.size();i++)
			nbrOfParameters+=vecMPI.get(i).get_nbrOfParameters();	
...
//			dmdT=zonen[i].get_dmi_dT();
			...
			if(dmi.length!=CP.SPEZIES_FABRIK.get_nbrOfSpecies()){
				try{
