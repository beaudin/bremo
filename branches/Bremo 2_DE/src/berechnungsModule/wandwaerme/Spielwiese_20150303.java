...
public class Spielwiese_20150303 extends WandWaermeUebergang{
	...
	protected Spielwiese_20150303(CasePara cp) {
...
//		qw=alpha*(T-T_piston);
	...
	public double get_WandWaermeStromDichtePiston(double time, Zone[] zonen_IN, double fortschritt){
		...
		double C_2w=0; //0.001; //0.00324;
		double C_k=0.03; //0.063; //0.0955; //0.066; //0.0955; //0.6; //0.01;
		...
		//double T=get_Tmb(zonen_IN);		//Mittlere Brennraumtemperatur
		double T_u=zonen_IN[0].get_T();		//Temperatur verbrannte Zone
		double T_b=zonen_IN[1].get_T();		//Temperatur verbrannte Zone
		...
		double KolbengeschwindigkeitsTerm=Math.pow(Kolbengeschwindigkeit, 2)/2;
		
		//double Verbrennungsterm=(1+WoschniTerm*C_w)*(1+KolbengeschwindigkeitsTerm*C_k);
		//double Verbrennungsterm=Math.pow((1+WoschniTerm*C_w), 0.8)*Math.pow((1+KolbengeschwindigkeitsTerm*C_k), 0.8);
		double Verbrennungsterm=Math.pow((1+KolbengeschwindigkeitsTerm*C_k), 0.8);
	...
	public double get_WandWaermeStromDichteHead(double time, Zone[] zonen_IN, double fortschritt){
		...
		double C_w=70; //110; //44; //40;
		double C_1w=0; //2.28+0.308*0;
		double C_2w=1E-6; //0.00324
		double C_k=0; //0.063;//0.6;
		double Kolbengeschwindigkeit= motor.get_Kolbengeschwindigkeit(time);
		...
		//double Verbrennungsterm=(1+WoschniTerm*C_w)*(1+KolbengeschwindigkeitsTerm*C_k);
		//double Verbrennungsterm=Math.pow((1+WoschniTerm*C_w), 0.8)*Math.pow((1+KolbengeschwindigkeitsTerm*C_k), 0.8);
		double Verbrennungsterm=Math.pow((1+WoschniTerm*C_w), 0.8);
		...
		if(motor.isHubKolbenMotor()){
			...
			qwk=alpha_u*(T_u-T_head)*Math.pow((1-fortschritt), 2)+alpha_b*(T_b-T_head)*(1-Math.pow((1-fortschritt), 2))*Verbrennungsterm;
			//qwk=alpha_u*(T_u-T_head)*(1-fortschritt)+alpha_b*(T_b-T_head)*fortschritt*Verbrennungsterm;
