...
public class SimpleDirectInjection extends Injection {
	...
	double dm_krst;	
	private FuelProperties krstProp;

	public SimpleDirectInjection(CasePara cp,int index) {
		super(cp,index);
		
		String vergleichsKraftstoff=CP.get_fuelPropsForEvap(index);
		krstProp =new FuelProperties(vergleichsKraftstoff);
...
//		}else 
		if(super.boi<super.CP.SYS.SIMULATION_START_SEC&&!super.IS_LWA_EINSPRITZUNG){	
			try {
				throw new ParameterFileWrongInputException("" +
						"Fuer das gewaehlte Einspritzmodell " +this.FLAG+ " der " +index+
		...
		super.mKrst_dampf.addValue(CP.SYS.SIMULATION_START_SEC,mKrst_verdampft );
	...
	public double get_dQ_fuelVapor(double time,Zone zn) {
		...
		double dm=this.get_diff_mFuel_vapor(time, zn);
...
//			+dm*this.get_cp_mass_fl(Tkrstfl)*(get_Tkrst_dampf(time,zn)-get_T_Krst_fl());
	...
	public double get_diff_mFuel_vapor(double time,Zone zn) {	
		if(time<super.eoi&&time>=super.boi){			
			return dm_krst;
		}else if(time<super.eoi+CP.SYS.CYCLE_DURATION_SEC&&time>=super.boi+CP.SYS.CYCLE_DURATION_SEC){
	...
	public void calculateIntegralvalues(double time, Zone zn) {		
		mKrst_verdampft=mKrst_verdampft+this.get_diff_mFuel_vapor(time, zn)*super.CP.SYS.WRITE_INTERVAL_SEC;		
