 ...
 * @author   hoenl, busch
 */
public class Motor_reciprocatingPiston extends Motor{
	...
	protected Motor_reciprocatingPiston(CasePara cp){
		super(cp);
		CP=super.CP;
		PLEUELLAENGE =CP.get_conRodLength();			//m
		BOHRUNG= CP.get_bore();						//m
		KURBELRADIUS = CP.get_crankRadius();		//m
		EPS=CP.get_compressionRatio();  					//-			
		SCHRAENKUNG =CP.get_pistonPinOffset();			//m
		DESACHSIERUNG =-1*CP.get_axialOffset();		//m	
		AUSLASSSCHLUSS =CP.get_EVO(); 		//°KWnZOT
		AUSLASSOEFFNET =CP.get_EVC(); 		//°KWnZOT
		EINLASSSCHLUSS =CP.get_IVC(); 		//°KWnZOT
		EINLASSOEFFNET =CP.get_IVO(); 		//°KWnZOT	
	...
	public double get_IV_Stroke(){
		if(Double.isNaN(ivStroke))
			ivStroke=CP.get_IV_Stroke_max();
		return ivStroke;
	}

	public double get_IV_Stroke_max(){
		if(Double.isNaN(ivStrokeMax))
			ivStrokeMax=CP.get_IV_Stroke_max();					//m
		return ivStrokeMax;
	}
 ...
 */
	...
	public double get_BrennraumFlaeche(double time) {			
		...
		double zylinderLaufflaeche =get_CylinderLinerArea(time);	
		brennraumFlaeche = get_fireDeckArea() + get_Kolbenflaeche() + 
		zylinderLaufflaeche;
		return brennraumFlaeche;
	}
	
	public double get_CylinderLinerArea(double time){
		double zylinderLaufflaeche = java.lang.Math.PI * get_bore() * 
				get_combustionChamberHeight(time);	
		return zylinderLaufflaeche;
	}
	
	/**
	 * Returns the sum of squish + the distance from the piston to TDC
	 * @param time [s]
	 * @return
	 */
	public double get_combustionChamberHeight(double time){
		return (get_Kolbenweg(time)+get_Quetschspalthoehe());
	}
	...
	public double get_FeuerstegFlaeche() {
		// A_Fst = 2 * d * pi * h_Fst
		double feuerstegFlaeche = 2*java.lang.Math.PI*get_bore()*get_Feuersteghoehe();		
