...
public class Hensel extends WallHeatTransfer {
...
//	protected final IndizierDaten indiD;
	...
	protected Hensel(CasePara cp) {
		...
		mittlereKolbengeschwindigkeit=cp.get_rotSpeedInRotperSec()*motor.get_Hub()*2; //[m/s]
		Twall=cp.get_T_Cyl();
	    t_zzp = 0;
	    p_zzp = 0;	       
	}
...
//			dichte = 1.2; // kg / m³; Dichte von Luft bei 20° angenommen => Vereinfachung, da entdrosselt und Dichte aus SAugrohr...
			...
			mluft= mTot*cp.get_rotSpeedInRotperSec()/2; // [kg/s]
			...
			Hashtable<Spezies,Double> abg=LHV_Calculator.calcMolenBruechePerfekteVerbrennung(cp, gg);
			double xCO2_Ex=abg.get(cp.SPECIES_FACTROY.get_CO2());			
			x_rg=xCO2/xCO2_Ex;	
	...
	private double getGeschwindigkeit(double time, double exp_m,double mluft,double dichte){
		...
		double querschnitt = Math.pow(motor.get_bore(),2)/4*Math.PI ;
...
//		verbrennungsterm = 1.0;
	...
	public double get_wallHeatFlux(double time, Zone[] zonen_IN, double fortschritt,VectorBuffer tBuffer){
		...
		double cT=10.0;
		double phi_Delay=cT*fortschritt*1000/(cp.get_rotSpeedInRotperSec()*60);
		double T=0;
