...
public class WoschniHuber extends WallHeatTransfer {
	...
	protected WoschniHuber(CasePara mp) {
		super(mp);		
		this.cp=super.cp;
		indiD=new IndicatorData(cp);	//TODO: umprogrammieren, so dass die Indizierdatei nur ein einziges Mal eingelesen wird
		...
		Druck_1=indiD.get_pZyl(cp.get_IVC()); //[bar]
		...
		refPunkt=cp.get_refTime_WoschniHuber();
	...
	public double get_WaermeUebergangsKoeffizient(double time, Zone[] zonen_IN, double fortschritt) {
		...
		if(!cp.SIMULATION_MODEL.isDVA()){
			try{
				throw new BirdBrainedProgrammerException("WoschniHuber is only vlid for pressure trace analyses");
