...
public class KivaPostProcessingThread implements Callable{
	...
	public void postProcess(){		
		...
		ggVec=new GasMixture[kpr.get_nbrOfCells()];
		...
		for(int cellIdx=0;cellIdx<ggVec.length;cellIdx++){	
			...
			ggVec[cellIdx]=new GasMixture(name);
		...
		GasMixture allCellSpec=new GasMixture("allCellSpec");
	...
	private void createBremoMultiZoneInput(int nbrOfZones){
		...
		for(int zIdx=0;zIdx<nbrOfZones;zIdx++){
			if(vecsIdx[zIdx].size()>0){//chk if there are zones for that lambda class
				...
				GasMixture gg=new GasMixture("z"+zIdx);
				...
				lamLHVmTotpVTmi[writeIdx][1]=gg.get_LHV_mass()*1e-6;
				...
				int nbrOfSpecies=CP.SPECIES_FACTROY.get_nbrOfSpecies();
				for(int idx=0;idx<nbrOfSpecies;idx++){
					Spezies spec=CP.SPECIES_FACTROY.get_Spez(idx);
