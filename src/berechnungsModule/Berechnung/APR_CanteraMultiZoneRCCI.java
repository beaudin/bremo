...
public class APR_CanteraMultiZoneRCCI extends APR_Cantera {		
	...
	protected APR_CanteraMultiZoneRCCI(CasePara cp) {
		...
		this.nbrOfZones=cp.get_anzAxialPakete(indexOfMainInj)*cp.get_anzRadialPakete(indexOfMainInj)+1;
		...
		axPakete=cp.get_anzAxialPakete(indexOfMainInj);
		radPakete=cp.get_anzRadialPakete(indexOfMainInj);
	...
	private Zone[] initFromFile(){
		...
		double [] molFrac=new double [CP.SPEZIES_FABRIK.get_NmbrOfAllSpez()];
		...
		for(int j=0;j<molFrac.length;j++){			
			ht.put(CP.SPEZIES_FABRIK.get_Spez(j), molFrac[j]);
		}			
	...
	private double calc_dma_Hanz(Zone z0,Zone z1, double time){	
		...
		double vh = ((Motor_HubKolbenMotor) motor).get_IV_Stroke(); // [m]
		double vh_max = ((Motor_HubKolbenMotor) motor).get_IV_Stroke_max(); // [m]
