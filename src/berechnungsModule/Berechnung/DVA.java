 ...
 *
 */
public abstract class DVA extends SimulationModel{

	private ResultsBuffer ergBuffDebug;
	...
	protected DVA(CasePara cp) {		
		super(cp,new ResultsBuffer(cp,"DVA_"));	
		indiD=new IndicatorData(cp);
		dQburn=0;
		ergBuffDebug=new ResultsBuffer(cp,"DVA_DEBUG_");
		precission=CP.get_precissionPressureTraceAnalysis();
		relaxFactor=CP.get_relaxFactor();
	}	

	public abstract Zone[] firstLawCombustionChamber(double time, Zone[] zonen);	
		...
		if(pDiff<=precission){				
			...
			if(this.dQIters==0){
				...
				for(int znIdx=0;znIdx<zonen.length;znIdx++){
					if(zonen[znIdx].get_m()>CP.SYS.MINIMUM_ZONE_MASS){
						Spezies gg=zonen[znIdx].get_gasMixtureZone();
	...
	protected double convert_dQburn_2_dmKrstBurn(double dQburn, Spezies frischGemisch,
		...
		abgas.set_Gasmixture_moleFracs(gg.get_GG_molenBrueche(p, Tu, frischGemisch));
	...
	protected boolean verbrennungHatBegonnen(double time, VectorBuffer dQ_in_){
		boolean burns=false;
		...
		if(CP.is_VerbrennungAutoDetect()){	
			if(foundVerbrennungsBeginn==false){
				...
				double dQb_buffer[]=new double[CP.SYS.NBR_OF_CALCULATED_VALUES];
				...
				do{
					...
					t0=t0+dt;	
					index+=1;
				}while(t0<=CP.SYS.SIMULATION_END_SEC);
