...
public class Zone {
	...
	private final ChemEquilibriumSolver GG_SOLVER; 		
	...
	public Zone(CasePara cp,double p_init,double V_init, double T_init, double m_init,
		...
		this.gg_Zone=new GasMixture("ggZone"+id);
		this.burns=burns;

		Hashtable<Spezies, Double> einzelMassen =new Hashtable<Spezies, Double>();

		if(gg.isGasGemisch()&&!CP.SPECIES_FACTROY.isToIntegrate(gg)){
			einzelMassen=this.get_einzelMassenHash(
					((GasMixture)gg).get_speziesMassenBruecheDetailToIntegrate(), m_init);
		}else{				
	...
	public void set_p_V_T_mi(double [] p_V_T_mi){
		...
		if(p_V_T_mi.length!=(CP.SPECIES_FACTROY.get_nbrOfSpecies()+3)){
			try{
		...
		for(int i=0; i<p_V_T_mi.length-3;i++) {
			if(p_V_T_mi[i+3]!=0){ //Abfrage ob mi <0 kommt in "set_p_V_T_mi(p_init,V_init,T_init,miHT)"
				miHT.put(CP.SPECIES_FACTROY.get_Spez(i), p_V_T_mi[i+3]);	
			}
		}
	...
	public void set_p_V_T_mi(double p_init,double V_init, double T_init,Hashtable<Spezies, Double> mi){			
		...
		if(p_Zone<=0||V_Zone<=0||T_Zone<=0||(((Double) p_Zone).isNaN())
			...
			while(e.hasMoreElements()){
				...
				mi_=mi.get(spez);
				if(mi_<0){	
					if(Math.abs(mi_)>=0.00001*CP.SYS.MINIMUM_ZONE_MASS){
						try{
			...
			if(this.m_Zone>0){
				...
				if(loeseChemGleichgwicht==true)
					gg_Zone.set_Gasmixture_moleFracs(GG_SOLVER.get_GG_molenBrueche(p_Zone, T_Zone, gg_Zone));	

			}else{
				Hashtable<Spezies, Double> massenBrueche =new Hashtable<Spezies, Double>();
				Spezies co2=CP.SPECIES_FACTROY.get_CO2();
			...
			//	System.out.println("(pV-mRT)/pV: "+(pV-mRT)/pV*100);
				...
				this.T_Zone=T_temp;
				if(chkT>7.555e-3&&m_Zone>CP.SYS.MINIMUM_ZONE_MASS){
					try{
	 ...
	 */
	public void set_dm_out(double dm_aus) throws NegativeMassException{
	...
	public void set_dm_aus(double dm_a,Spezies spez) throws NegativeMassException{		
		...
		if(dm_a>0){				
			...
			if(spez.isGasGemisch()&& CP.SPECIES_FACTROY.isToIntegrate(spez)==false){
				Hashtable<Spezies, Double> massenBruchHash_aus=	
					((GasMixture)spez).get_speziesMassenBruecheDetailToIntegrate();
			...
			while(e.hasMoreElements()){
				spez1=e.nextElement();
				if(!CP.SPECIES_FACTROY.isToIntegrate(spez1)){
					try{
...
//				System.out.println("Zone.set_dm_aus: dm_a !=dmCheck " + ((dmCheck-dm_a)) );
	...
	public void set_dm_in(double dm_zu, double T_zu, Spezies spez_zu){
		if(dm_zu<0){
		...
		if(dm_zu!=0){//wenn die Masse null ist mach garnichts				
			...
			if(spez_zu.isGasGemisch()&& CP.SPECIES_FACTROY.isToIntegrate(spez_zu)==false){

				Hashtable<Spezies, Double> massenBruchHash_zu=	
					((GasMixture)spez_zu).get_speziesMassenBruecheDetailToIntegrate();
			...
			while(e.hasMoreElements()){
				spez=e.nextElement();
				if(!CP.SPECIES_FACTROY.isToIntegrate(spez)){
					try{
...
//				System.out.println("Zone.set_dm_ein: dm_zu !=dmCheck " +((dmCheck-dm_zu)));			
	...
	public void set_dQ_in_out(double dQ_zu){
		sumdQ=sumdQ+dQ_zu;
	}
	...
	private  double[] calc_dudT_dRdT(){
		...
		if(loeseChemGleichgwicht==true && T_Zone>=GG_SOLVER.get_T_Freeze() ){
			GasMixture zoneGG =
				new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone, T_Zone, gg_Zone),"zoneGG"+ID);
			GasMixture rauchgas =
				new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone, T_Zone-delta_T, gg_Zone),"rauchgasZone"+ID);
	...
	private double[] calc_dudp_dRdp(){
		...
		if(loeseChemGleichgwicht==true && T_Zone>=GG_SOLVER.get_T_Freeze()){
			GasMixture zoneGG =
				new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone, T_Zone, gg_Zone),"zoneGG"+ID);
			GasMixture rauchgas =
				new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone+delta_p, T_Zone, gg_Zone),"rauchgasZone"+ID);
	...
	private Hashtable<Spezies,Double> calc_dmj_dp(){
		...
		if(loeseChemGleichgwicht==true&&burns==true){
			double delta_p=5000;
			GasMixture rauchgas =new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone+delta_p,T_Zone, gg_Zone),"rauchgas_temp_"+ID);				
	...
	private Hashtable<Spezies,Double> calc_dmj_dT(){
		...
		if(loeseChemGleichgwicht==true&&burns==true){
			...
			GasMixture rauchgas =new GasMixture(GG_SOLVER.get_GG_molenBrueche(p_Zone, T_Zone-delta_T, gg_Zone),"rauchgas");					
			Hashtable<Spezies,Double> rgSpezMassenBrueche=rauchgas.get_speziesMassenBruecheDetail();	
	...
	public double [] get_dmi_dp() {
		...
		double dmi []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];
		Enumeration<Spezies> e=dmdp.keys();
		Spezies spez;
		while(e.hasMoreElements()){
			spez=e.nextElement();
			dmi[CP.SPECIES_FACTROY.get_indexOf(spez)]=dmdp.get(spez);				
		}	
		return dmi;
	}
	...
	public double [] get_dmi_dT() {		
		...
		while(e.hasMoreElements()){
			spez=e.nextElement();
			dmi[CP.SPECIES_FACTROY.get_indexOf(spez)]=dmdT.get(spez);				
		}	
		return dmi;
	}
	...
	public double [] get_mi(){
		...
		double mi []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];
		Enumeration<Spezies> e=mb.keys();
		Spezies spez;
		while(e.hasMoreElements()){
			spez=e.nextElement();
			mi[CP.SPECIES_FACTROY.get_indexOf(spez)]=mb.get(spez)*m_Zone;				
		}			
		return mi;
	}	
	...
	public double [] get_ci(){
		...
		Hashtable<Spezies, Double> mb=gg_Zone.get_speziesMassenBruecheDetailToIntegrate();
		double ci []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];		
		Enumeration<Spezies> e=mb.keys();
		Spezies spez;
		while(e.hasMoreElements()){
			spez=e.nextElement();
			ci[CP.SPECIES_FACTROY.get_indexOf(spez)]=mb.get(spez)*m_Zone/spez.get_M()/this.V_Zone;				
		}			
		return ci;
	}	
	...
	public double [] get_dmi(){
		...
		double dmi1 []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];
		Enumeration<Spezies> e=dmj_ein.keys();
		Spezies spez;
		while(e.hasMoreElements()){
			spez=e.nextElement();
			dmi1[CP.SPECIES_FACTROY.get_indexOf(spez)]=dmj_ein.get(spez);				
		}
		double dmi2 []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];
		e=null;		
		e=dm_aus.keys();
		while(e.hasMoreElements()){
			spez=e.nextElement();
			dmi2[CP.SPECIES_FACTROY.get_indexOf(spez)]=this.dm_aus.get(spez);
...
//			System.out.println("sumdmi in Zone " +this.ID +": " +sumdmi);
	...
	public double [] get_MoleFractions(){
		...
		double x []=new double[CP.SPECIES_FACTROY.get_nbrOfSpecies()];
		Enumeration<Spezies> e=mf.keys();
		Spezies spez;
		while(e.hasMoreElements()){
			spez=e.nextElement();
			x[CP.SPECIES_FACTROY.get_indexOf(spez)]=mf.get(spez);				
		}			
		return x;
	}
	...
	public double [] get_p_V_T_mi(){			
		double[]p_V_T_mi=new double [3+CP.SPECIES_FACTROY.get_nbrOfSpecies()];
	 ...
	 * @return
	 */
	public GasMixture get_gasMixtureZone() {
		return gg_Zone;
	}
	...
	public void massenElementZumischen(double mZu, Spezies sZu){
		...
		if(mZu<0){ 
			if(sZu.isGasGemisch()){				
				einzelMassenHashIn	= //enthaelt negative Werte!!
					this.get_einzelMassenHash(((GasMixture)sZu).get_speziesMassenBruecheDetail(), mZu);	
			}else{
		...
		molenBruchHash=GasMixture.Gasmischer.massenBruch2molenBruch(massenBruchHashNeu);

		GasMixture gasGemisch = new GasMixture(molenBruchHash,"");	
		//würde man hier die 
		gg_Zone.set_Gasmixture_moleFracs(gasGemisch.get_speziesMolenBruecheDetail());
		m_Zone=m_Zone_neu;
	...
	public void zoneZumischen(Zone zone2){
		...
		Spezies s_Zu=zone2.get_gasMixtureZone(); 
	...
	public static Zone zonenMischen(CasePara cp,Zone z1, Zone z2,boolean burns, int ID){
		...
		double T1=z1.get_T();
		double V1=z1.get_V();
		Spezies s1=z1.get_gasMixtureZone(); 

		double m2=z2.get_m();
		double T2=z2.get_T();
		double V2=z2.get_V();
		Spezies s2=z2.get_gasMixtureZone(); 

		//Erzeugen einer Spezies die durch die Mischung der beiden ZonenSpezies entsteht
		GasMixture s0=new GasMixture("zonenGemisch");
