...
public class PostProcessor {
	...
	private ResultsBuffer ergB;
	
	public PostProcessor(VectorBuffer dm_buffer,VectorBuffer dQb_buffer,VectorBuffer dQw_buffer, CasePara cp){
			this.CP=cp;
			ergB=new ResultsBuffer(CP,"DVA_Post_");
			inti=new Integrator();
...
//			txtFile.writeMatrixToFile(MatLibBase.transp_2d_array(matrix), false);			
			...
			double mAGR_inter=CP.RESIDUAL_GAS_MODEL.get_mResidualGas_perCycle();
			i+=1;
	...
	private double [] clacVerlustteilung(double x50,double Qw,double pmi){
		...
		double Hu=me.get_spezKrstALL().get_LHV_mass();	
		double mKrstAll=me.get_mFuel_Sum_Cycle();
		double Q=mKrstAll*Hu;
		double Qmax=me.get_mFuel_Sum_Cycle()*me.get_spezKrstALL().get_LHV_mass();	
		///////////////////////////////////
		...
		abgas.set_Gasmixture_moleFracs(
		...
		if(m.isHubKolbenMotor()){
			Vh=((Motor_reciprocatingPiston)m).get_Hubvolumen();		
		}
	...
	private double [] findUmsatzPunkte(VectorBuffer dm_buffer){
