...
public class Chang extends WallHeatTransfer {
	...
	public double get_WaermeUebergangsKoeffizient(double time, Zone[] zonen_IN,double fortschritt) {
		if(setRefConditions){
			...
			if(time!=cp.get_IVC()){//simulation start is not at IVC (e.g homogeneous multizone model
				...
				if(cp.compareToExp()){
					IndicatorData indiD=new IndicatorData(cp);
		...
		double bdh=motor.get_Kompressionsvolumen()*4/Math.PI/Math.pow(motor.get_bore(), 2);
