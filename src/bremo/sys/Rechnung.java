...
public class Rechnung {
	...
	private double turbulence = 0; // Turbulenzfaktor unten (falls Bargende)
									...
									// positiv sein. Falls nicht verändert wird
									// Fehler abgefangen
		...
		if (CP.MODUL_VORGABEN.get("Wandwaermemodell").equals("Bargende")
				|| CP.MODUL_VORGABEN.get("Wandwaermemodell").equals(
						"BargendeHeinle")) { // vor der do-while-Schleife, sonst
												// würde bei jedem Schritt
												// "equals("Bargende")" im
												// CP-File abgefragt werden
			bargende = true;
		}
		if (CP.MODUL_VORGABEN.get("Wandwaermemodell").equals("BargendeFVV")) { // für
																				// Bargende
																				// nach
																				// FVV
			fvv = true;
	...
	public void berechnungDurchfuehren() {
		try {
			...
			// Anzahl simWerte muss für eine APR (minus 1) angepasst werden um
			// einen
			...
			for (int i = 1; i < anzSimWerte; i++) {
				...
				if (dglSys.isDVA() == true) {
					...
					// Später soll turbulence den entsprechenden Wert
					// überschreiben
					// (bisher nur für WWÜ Bargende nötig)
					// if
					// (CP.MODUL_VORGABEN.get("Wandwaermemodell").equals("Bargende")){
					// //vor der do-while-Schleife, sonst würde bei jedem
					// Schritt
					...
					do {
						...
						// Überschreiben von Turbulenzfaktor k (siehe oben)
					...
					// Thermodynamisches Verdichtungsverhältnis nach der
