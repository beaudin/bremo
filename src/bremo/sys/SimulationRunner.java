...
public class SimulationRunner {		
	...
	public SimulationRunner(CasePara cp) {		
		CP=cp;		
	}
...
//		Solver sol=new Solver(CP, dglSys);
		SimulationModel dglSys=CP.SIMULATION_MODEL;
		Solver sol=CP.SOLVER;
		// initial value of x
		x0 =CP.SYS.SIMULATION_START_SEC; //sollte normalerweise null sein aber man weiss ja nie
		sol.setInitialValueOfX(x0);

		// final value of x
		xn = CP.SYS.SIMULATION_END_SEC; //in [s]  
		...
		int anzSimWerte=CP.SYS.NBR_OF_CALCULATED_VALUES;
		double time;
		Zone[] zn=dglSys.get_initialZones();
		dglSys.bufferResults(x0, zn);
