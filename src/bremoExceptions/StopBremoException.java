...
public abstract class StopBremoException extends Exception {
	...
	public void stopBremo() {
		...
		// Dieser Befehl wird benoetigt um das Programm zu stoppen wenn es nicht
		// als swing app laeuft
		// LogFileWriter.addItemToLog(super.getMessage());
		try {
			log_Message();
			AusgabeSteurung.Error(Separator
					+ "!!!!   Bremo wurde gestopt    !!!!" + Separator);

		} catch (Exception e) {

		}
		throw new InvalidParameterException(Separator + super.getMessage()
				+ Separator);
	}
