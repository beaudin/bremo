...
public class IndicatorFileReader_txt extends IndicatorFileReader{
	...
	public IndicatorFileReader_txt(CasePara cp,String pfad,
			int kanal_pZyl,
			int kanal_pEin,
			int kanal_pAus){	
		super(cp);
	...
	public IndicatorFileReader_txt(CasePara cp,String fileName,int kanal_pZyl){		
		super(cp);
		dreiDruecke=false;
		spalte_pZyl = kanal_pZyl;
	...
	private double[][] readFile(int abZeile) {
		...
		if(zeitEinheit.equals("[KWnZOT]")){
			...
			if((t_MAX-t_MIN)-CP.SYS.CYCLE_DURATION_CA>2){
				try {
